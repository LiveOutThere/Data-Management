...
SELECT  
	...
	REPLACE(dbo.getPATUrlKey('patagonia-' + dbo.getPATName(a.[Style Description]) + '-' + dbo.getPATColor(a.[color description]) + '-' + a.size + '-' + dbo.getPATGender(a.[Style Description]) + '-' + 'fw13a'),'--','-') AS url_key,
...
SELECT DISTINCT
	...
	'FW13A-PAT-' + CAST(a.style AS nvarchar) AS sku,
	...
	REPLACE(LOWER('Patagonia-' + dbo.getPATUrlKey(dbo.getPATName(a.name) + '-' + dbo.getPATGender(a.name) + '-FW13a')),'--','-') AS url_key,
	'Patagonia ' + dbo.getPATName(a.name) + ' - ' + dbo.getPATGender(a.name) + '''s' AS meta_title,
...
UPDATE tbl_LoadFile_FW13_PAT SET
	categories = CASE WHEN (SELECT TOP 1 categories FROM [LOT_Inventory].dbo.tbl_Magento_Categories WHERE vendor_product_id = a.vendor_product_id) IS NULL THEN 'Uncategorized' END,
...
UPDATE tbl_LoadFile_FW13_PAT SET categories = CASE WHEN categories <> 'Uncategorized' THEN categories + ';;' + manufacturer + '/' + REPLACE(categories,';;',';;' + manufacturer + '/') ELSE 'Uncategorized' END
