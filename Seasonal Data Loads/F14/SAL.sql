...
SELECT DISTINCT 
	...
	,WHSL											AS cost
	,dbo.getSALDepartment							AS department
...
select distinct REPLACE([SAP Number],'L','') from tbl_RawData_FW14_SAL_Marketing 
select RIGHT([SAP Number],2) from tbl_RawData_FW14_SAL_Marketing 

select distinct [ITEM No] from tbl_RawData_FW14_SAL_UPC_Price
select RIGHT([ITEM No],2) from tbl_RawData_FW14_SAL_UPC_Price

select * from tbl_RawData_FW14_SAL_Marketing
select * from tbl_RawData_FW14_SAL_UPC_Price
select distinct COLOUR from tbl_RawData_FW14_SAL_UPC_Price

select distinct 

select distinct LEFT(REPLACE([SAP Number],'L',''),6) from tbl_RawData_FW14_SAL_Marketing
select distinct LEFT([ITEM No],6) from tbl_RawData_FW14_SAL_UPC_Price

select distinct a.[SAP Number],b.[ITEM No] 
from tbl_RawData_FW14_SAL_Marketing as a
inner join tbl_RawData_FW14_SAL_UPC_Price as b
on REPLACE(a.[SAP Number],'L','') = b.[ITEM No] 

--UPDATE tbl_LoadFile_FW14_SAL SET choose_size = REPLACE(choose_size,'.5','½')

select distinct [Color 1], [Color 2], [Color 3] from tbl_RawData_FW14_SAL_Marketing
...
WHERE b.Brand = 'SAL' AND a.type ='simple'

UPDATE a SET
	a.image = b.image
FROM tbl_LoadFile_FW14_SAL AS a 
INNER JOIN tbl_LoadFile_FW13_SAL AS b
...
UPDATE tbl_LoadFile_FW14_SAL SET
	...
	description		= dbo.getSALLDesc(a.name,a.vendor_product_id),
	features		= dbo.getSALFeatures(a.name,a.vendor_product_id)
FROM tbl_LoadFile_FW14_SAL AS a
