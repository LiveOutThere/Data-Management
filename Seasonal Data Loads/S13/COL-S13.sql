...
SELECT  dbo.getMagentoSimpleSKU('SS13A-COL', CAST(CAST(a.Material AS bigint) AS varchar(255)), a.[Color Code], dbo.getCOLSize(a.Size,a.Dim)) AS sku,
		a.Material AS vendor_product_id,
		dbo.getCOLName(b.Name) AS name,
		dbo.getCOLGender(b.Name,b.[Collection]) AS gender,
		a.[Color Name] AS choose_color,
		dbo.getCOLSize(a.Size,a.Dim) AS choose_size,
		a.[Color Code] AS vendor_color_code,
		dbo.getCOLSize(a.Size,a.Dim) AS vendor_size_code,
		a.UPC AS vendor_sku,
		CAST(CEILING(b.MSRP) AS float) - 0.01 AS price,
		b.Cost AS cost,
		0 AS has_options,
		'simple' AS type,
		a.[Color Name] AS image_label,
		dbo.getUrlKey(dbo.getCOLName(a.Description), 'Columbia', a.[Color Name] + ' - ' + dbo.getCOLSize(a.Size,a.Dim), dbo.getCOLGender(b.Name,b.Collection)) + '-ss13a' AS url_key
FROM tbl_RawData_SS13_COL_MHW_UPC AS a
INNER JOIN tbl_RawData_SS13_COL_PriceList AS b
ON a.Material = b.SAP
WHERE a.Brand = 'COL'
