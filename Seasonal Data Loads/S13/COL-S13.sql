...
SELECT  dbo.getMagentoSimpleSKU('SS13A-COL', c.SAP, a.[Color Code], ISNULL(b.Size,a.Size)) AS sku,
		c.SAP AS vendor_product_id,
		ISNULL(b.[Display Name],a.Description) AS name,
		...
		a.UPC AS vendor_sku,
		CAST(CEILING(c.MSRP) AS float) - 0.01 AS price,
		c.Cost AS cost,
		0 AS has_options,
		'simple' AS type,
		a.[Color Name] AS image_label,
		dbo.getUrlKey(ISNULL(b.[Display Name],a.Description), 'Columbia', a.[Color Name] + ' - ' + ISNULL(b.Size,a.Size), dbo.getCOLGender(c.Collection)) + '-ss13a' AS url_key
FROM tbl_RawData_SS13_COL_MHW_UPC AS a
LEFT JOIN tbl_RawData_SS13_COL_MHW_Catalog AS b
ON a.Material = b.Material AND b.Brand = 'Columbia' AND a.Brand = 'COL'
INNER JOIN tbl_RawData_SS13_COL_PriceList AS c
ON a.Material = c.SAP WHERE a.Brand = 'COL'
