...
IF EXISTS (SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[tbl_LoadFile_S13_MAR]')
AND OBJECTPROPERTY(id, N'IsUserTable') = 1)
DROP TABLE [dbo].[tbl_LoadFile_S13_MAR]

CREATE TABLE [dbo].[tbl_LoadFile_S13_MAR](
	[id] [int] IDENTITY(1,1) NOT NULL,
	[store] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL CONSTRAINT [DF_tbl_LoadFile_S13_MAR_store]  DEFAULT ('admin'),
	[websites] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL CONSTRAINT [DF_tbl_LoadFile_S13_MAR_websites]  DEFAULT ('base'),
	[type] [nvarchar](50) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,
	[sku] [nvarchar](50) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,
	[name] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,
	[categories] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,
	[attribute_set] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL CONSTRAINT [DF_tbl_LoadFile_S13_MAR_attribute_set]  DEFAULT ('default'),
	[configurable_attributes] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,
	[has_options] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,
	[price] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,
	[cost] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,
	[status] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL CONSTRAINT [DF_tbl_LoadFile_S13_MAR_status]  DEFAULT ('Enabled'),
	[tax_class_id] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL CONSTRAINT [DF_tbl_LoadFile_S13_MAR_tax_class]  DEFAULT ('Taxable Goods'),
	[gender] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,
	[visibility] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL CONSTRAINT [DF_tbl_LoadFile_S13_MAR_visibility]  DEFAULT ('Not Visible Individually'),
	...
	[season] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL CONSTRAINT [DF_tbl_LoadFile_S13_MAR_season]  DEFAULT (N'FW12A'),
	...
	[manufacturer] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL CONSTRAINT [DF_tbl_LoadFile_S13_MAR_manufacturer]  DEFAULT ('Marmot'),
	[qty] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL CONSTRAINT [DF_tbl_LoadFile_S13_MAR_qty]  DEFAULT ((0)),
	[is_in_stock] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL CONSTRAINT [DF_tbl_LoadFile_S13_MAR_is_in_stock]  DEFAULT ((0)),
	...
	[backorders] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL CONSTRAINT [DF_tbl_LoadFile_S13_MAR_backorders]  DEFAULT ((0)),
	[manage_stock] [nvarchar](4000) COLLATE SQL_Latin1_General_CP1_CI_AS NULL CONSTRAINT [DF_tbl_LoadFile_S13_MAR_manage_stock]  DEFAULT ((1))
 CONSTRAINT [PK_tbl_LoadFile_S13_MAR] PRIMARY KEY CLUSTERED 
(
	[id] ASC
...
CREATE NONCLUSTERED INDEX [IX_tbl_LoadFile_S13_MAR] ON [dbo].[tbl_LoadFile_S13_MAR] 
(
	[sku] ASC,
	[type] ASC,
...
INSERT INTO tbl_LoadFile_S13_MAR (
		sku,
		vendor_product_id,
		[name],
		gender,
...
SELECT DISTINCT
	   dbo.getMagentoConfigurableSKU('FW12A-MAR', a.[Style Number]) AS sku,
		...
		dbo.getMARName(a.[Style Description]) AS name,
		CASE WHEN a.[Style Description] LIKE 'Wm''s%' THEN 'Women' WHEN a.[Style Description] LIKE 'Girl''s%' THEN 'Girl' WHEN a.[Style Description] LIKE 'Boy''s%' THEN 'Boy' WHEN a.[Style Description] LIKE 'Kid%' THEN 'Boy|Girl' ELSE 'Men' END AS gender,
		(SELECT MAX(price) FROM tbl_LoadFile_S13_MAR WHERE vendor_product_id = a.[Style Number]) AS price, -- the price for the configurable is always the highest-priced simple
		(SELECT MAX(cost) FROM tbl_LoadFile_S13_MAR WHERE vendor_product_id = a.[Style Number]) AS cost,
		'1' AS has_options,
...
DELETE FROM tbl_LoadFile_S13_MAR WHERE cost = '0.00'

UPDATE tbl_LoadFile_S13_MAR SET name = REPLACE(name,'Men''s ','') WHERE name LIKE '% 1P%' OR name LIKE '% 2P%' OR name LIKE '% 3P%' OR name LIKE '% 4P%' OR name LIKE '% 6P%' OR name LIKE '% 8P%' OR name LIKE '% Fly' OR name LIKE '% Footprint' OR name LIKE '% Tent %' OR name LIKE '% Sack' OR name LIKE '% Loft'

UPDATE       tbl_LoadFile_S13_MAR SET gender='Men|Women'
WHERE     (type = 'configurable') AND (gender = 'Men') AND (name NOT LIKE 'Men%')

UPDATE tbl_LoadFile_S13_MAR SET image = 'mar.jpg' WHERE (type = 'simple') AND (image IS NULL) AND (name LIKE '%footprint%')
...
UPDATE a SET
	image = (SELECT TOP 1 image FROM tbl_LoadFile_S13_MAR WHERE name = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(a.name, ' - ','-'), '-Tall',''), '-Short',''), '-Long',''),'-Large',''),'-X Large',''),' Long X-Wide','') AND vendor_color_code = a.vendor_color_code ORDER BY choose_color DESC)
FROM tbl_LoadFile_S13_MAR AS a
...
UPDATE a SET
	description = (SELECT [Positioning Statement] FROM tbl_RawData_S13_MAR_Additional WHERE [Style] = a.vendor_product_id),
	features = dbo.getMARFeatures(a.vendor_product_id),
	fabric = (SELECT [Materials] FROM tbl_RawData_S13_MAR_Additional WHERE [Style] = a.vendor_product_id),
	image = (SELECT TOP 1 image FROM tbl_LoadFile_S13_MAR WHERE vendor_product_id = a.vendor_product_id AND a.image IS NOT NULL ORDER BY choose_color DESC),
	image_label = (SELECT TOP 1 image_label FROM tbl_LoadFile_S13_MAR WHERE vendor_product_id = a.vendor_product_id AND a.image IS NOT NULL ORDER BY choose_color DESC),
...
UPDATE tbl_LoadFile_S13_MAR SET image = REPLACE(image,'+',''), small_image = REPLACE(small_image,'+',''), thumbnail = REPLACE(thumbnail,'+','') WHERE image IS NOT NULL
UPDATE tbl_LoadFile_S13_MAR SET thumbnail = '+' + image, small_image = '+' + image WHERE image IS NOT NULL
UPDATE tbl_LoadFile_S13_MAR SET image = '+' + image WHERE image IS NOT NULL
GO

CREATE VIEW [dbo].[view_LoadFile_S13_MAR]
AS
...
FROM dbo.tbl_LoadFile_S13_MAR AS a WHERE a.image IS NOT NULL
GO

DECLARE @sql varchar(1024)
SELECT @sql = 'bcp "SELECT * FROM LOT_Inventory.dbo.view_LoadFile_S13_MAR" queryout "C:\Data\Shared\MAR.csv" -w -t , -T -S ' + @@servername
EXEC master..xp_cmdshell @sql

--Remember to open the CSV up in TextMate and save with UTF-8 encoding and LF line breaks!

DROP VIEW view_LoadFile_S13_MAR