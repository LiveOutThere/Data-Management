...
(SELECT DISTINCT vendor_product_id FROM tbl_LoadFile_SS13_COL WHERE type = 'configurable' AND CAST(price AS float) <= 0 OR price IS NULL OR cost IS NULL) AS x

SET @count = (SELECT COUNT(DISTINCT vendor_product_id) FROM tbl_LoadFile_SS13_COL WHERE type = 'configurable' AND CAST(price AS float) <= 0 OR price IS NULL OR cost IS NULL)
...
(SELECT vendor_product_id + ' (' + name + ' - ' + vendor_color_code + ')' AS name FROM tbl_LoadFile_SS13_COL
...
(SELECT sku, vendor_sku
 FROM tbl_LoadFile_SS13_COL AS a WHERE (SELECT COUNT(*) FROM tbl_LoadFile_SS13_COL WHERE sku = a.sku) > 1) AS x
...
PRINT ''
SELECT @output = COALESCE(@output + CHAR(13) + '		', '') + name FROM
(SELECT name + ' (' + vendor_product_id + ')' AS name FROM tbl_LoadFile_SS13_COL WHERE type = 'configurable' AND (description IS NULL OR description = '')) AS x

SET @count = (SELECT COUNT(*) FROM tbl_LoadFile_SS13_COL WHERE type = 'configurable' AND (description IS NULL OR description = ''))
...
PRINT ''
SELECT @output = COALESCE(@output + CHAR(13) + '		', '') + name FROM
(SELECT DISTINCT vendor_product_id + '-' + vendor_color_code  + ' (' + choose_color + ')' AS name FROM tbl_LoadFile_SS13_COL WHERE type = 'simple' AND (image = '' OR image IS NULL)) AS x

SET @count = (SELECT COUNT(DISTINCT vendor_product_id + ' ' + choose_color) FROM tbl_LoadFile_SS13_COL WHERE type = 'simple' AND (image = '' OR image IS NULL))
...
PRINT ''
SELECT @output = COALESCE(@output + CHAR(13) + '		', '') + name FROM
(SELECT name FROM tbl_LoadFile_SS13_COL WHERE type = 'configurable' AND categories IS NULL OR categories = 'Uncategorized') AS x

SET @count = (SELECT COUNT(*) FROM tbl_LoadFile_SS13_COL WHERE type = 'configurable' AND categories IS NULL OR categories = 'Uncategorized')
...
(SELECT a.sku AS sku FROM tbl_LoadFile_SS13_COL AS a LEFT JOIN tbl_LoadFile_SS13_COL AS b
...
PRINT ''
SELECT @output = COALESCE(@output + CHAR(13) + '		', '') + name FROM
(SELECT name + ' (' + vendor_product_id + ')' AS name FROM tbl_LoadFile_SS13_COL WHERE type = 'configurable' AND (features IS NULL OR features = '')) AS x

SET @count = (SELECT COUNT(*) FROM tbl_LoadFile_SS13_COL WHERE type = 'configurable' AND (features IS NULL OR features = ''))
