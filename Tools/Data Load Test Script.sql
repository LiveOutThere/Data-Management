...
(SELECT vendor_product_id FROM tbl_LoadFile_SS13_COL WHERE type = 'configurable' AND CAST(price AS float) <= 0 OR price IS NULL) AS x

SET @count = (SELECT COUNT(*) FROM tbl_LoadFile_SS13_COL WHERE type = 'configurable' AND CAST(price AS float) <= 0 OR price IS NULL)
