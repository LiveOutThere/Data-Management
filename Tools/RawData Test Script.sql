...
(SELECT DISTINCT Material FROM tbl_RawData_SS13_MHW WHERE CAST(MSRP AS float) <= 0 OR MSRP IS NULL OR Base IS NULL) AS x

SET @count = (SELECT COUNT(DISTINCT Material) FROM tbl_RawData_SS13_MHW WHERE CAST(MSRP AS float) <= 0 OR MSRP IS NULL OR Base IS NULL)
...
(SELECT CAST(CAST(Material AS bigint) AS varchar(55)) + ' (' + [Display Name] + ' - ' + [Color Code] + ')' AS name FROM tbl_RawData_SS13_MHW
			  WHERE Size <> 'O/S'
			  GROUP BY [Display Name], Material, [Color Code]
			  HAVING COUNT(*) = 1) AS x
			  
SET @count = (SELECT COUNT(*) FROM (SELECT [Display Name], Material, [Color Code]
			 FROM tbl_RawData_SS13_MHW
			 WHERE Size <> 'O/S'
			 GROUP BY [Display Name], Material, [Color Code]
			 HAVING COUNT(*) = 1) AS y)
...
(SELECT (CAST(SKU AS nvarchar(50)) + ' ' + CAST(CAST(UPC AS bigint) AS nvarchar(50))) AS bumniglund
 FROM tbl_RawData_SS13_MHW AS a WHERE (SELECT COUNT(*) FROM tbl_RawData_SS13_MHW WHERE SKU = a.SKU) > 1) AS x
