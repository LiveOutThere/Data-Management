...
(SELECT DISTINCT vendor_product_id FROM tbl_LoadFile_SS14_ARC WHERE type = 'configurable' AND CAST(price AS float) <= 0 OR price IS NULL OR cost IS NULL) AS x

SET @count = (SELECT COUNT(DISTINCT vendor_product_id) FROM tbl_LoadFile_SS14_ARC WHERE type = 'configurable' AND CAST(price AS float) <= 0 OR price IS NULL OR cost IS NULL)
...
(SELECT name
 FROM (SELECT name + ' (' + department + ')' AS name, COUNT(*) AS total FROM tbl_LoadFile_SS14_ARC WHERE type = 'configurable' GROUP BY name + ' (' + department + ')') AS a WHERE a.total > 1) AS x
...
PRINT ''
SELECT @output = COALESCE(@output + CHAR(13) + '		', '') + name FROM
(SELECT vendor_product_id + ' (' + name + ' - ' + vendor_color_code + ')' AS name FROM tbl_LoadFile_SS14_ARC
...
(SELECT sku
 FROM (SELECT sku, COUNT(*) AS total FROM tbl_LoadFile_SS14_ARC GROUP BY sku HAVING MAX(type) = 'simple') AS a WHERE a.total > 1) AS x
...
(SELECT a.sku AS sku FROM tbl_LoadFile_SS14_ARC AS a LEFT JOIN tbl_LoadFile_SS14_ARC AS b
...
PRINT ''
SELECT @output = COALESCE(@output + CHAR(13) + '		', '') + name FROM
(SELECT name + ' (' + vendor_product_id + ')' AS name FROM tbl_LoadFile_SS14_ARC WHERE type = 'configurable' AND (features IS NULL OR features = '')) AS x

SET @count = (SELECT COUNT(*) FROM tbl_LoadFile_SS14_ARC WHERE type = 'configurable' AND (features IS NULL OR features = ''))
...
PRINT ''
SELECT @output = COALESCE(@output + CHAR(13) + '		', '') + name FROM
(SELECT name + ' (' + vendor_product_id + ')' AS name FROM tbl_LoadFile_SS14_ARC WHERE type = 'configurable' AND (fabric IS NULL OR fabric = '')) AS x

SET @count = (SELECT COUNT(*) FROM tbl_LoadFile_SS14_ARC WHERE type = 'configurable' AND (fabric IS NULL OR fabric = ''))
...
PRINT ''
SELECT @output = COALESCE(@output + CHAR(13) + '		', '') + name FROM
(SELECT name + ' (' + vendor_product_id + ')' AS name FROM tbl_LoadFile_SS14_ARC WHERE type = 'configurable' AND (description IS NULL OR description = '')) AS x

SET @count = (SELECT COUNT(*) FROM tbl_LoadFile_SS14_ARC WHERE type = 'configurable' AND (description IS NULL OR description = ''))
...
PRINT ''
SELECT @output = COALESCE(@output + CHAR(13) + '		', '') + name FROM
(SELECT DISTINCT vendor_product_id + '-' + vendor_color_code  + ' (' + choose_color + ')' AS name FROM tbl_LoadFile_SS14_ARC WHERE type = 'simple' AND (image = '' OR image IS NULL)) AS x

SET @count = (SELECT COUNT(DISTINCT vendor_product_id + ' ' + choose_color) FROM tbl_LoadFile_SS14_ARC WHERE type = 'simple' AND (image = '' OR image IS NULL))
IF @count <> 0 BEGIN
...
PRINT ''
SELECT @output = COALESCE(@output + CHAR(13) + '		', '') + name FROM
(SELECT name FROM tbl_LoadFile_SS14_ARC WHERE type = 'configurable' AND categories IS NULL OR categories = 'Uncategorized') AS x

SET @count = (SELECT COUNT(*) FROM tbl_LoadFile_SS14_ARC WHERE type = 'configurable' AND categories IS NULL OR categories = 'Uncategorized')
